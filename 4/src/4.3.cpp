...
{
	const int N_steps_per_block = 10000;
	...
	NVE sim = NVE("config/4.3-input-" + phase + ".dat", "config/4.3-config-" + phase + ".0", old);		//Inizialization
	if (string(argv[1]) == "start")
		sim.info();
	...
	ofstream ave;
	if (measure) {
		ave.open("data/4.3-averages-" + phase + ".dat", ios::app);
		if (!ave.is_open()) {
			cerr << "Could not open data/4.3-averages-" + phase + ".dat\n";
			return 1;
		}
	}
	out.open("data/4.3-measures-" + phase + ".dat", ios::app);
	if (!out.is_open()) {
		cerr << "Could not open data/4.3-measures-" + phase + ".dat\n";
		return 1;
	}
