...
double psi_trial_second(double x)
{
	return 0.0;
}

double psi_mu_sigma(double x)
{
	double mu = 0.83;
	double sigma = 0.61;
...
double psi_mu_sigma_second(double x)
{
	double mu = 0.83;
	double sigma = 0.61;
	double phi_minus = (x - mu) / sigma;
...
opportunely initialized by the initialize() function. */
	...
	consoleOutput();
	finalizePotentialEstimator(output_filename);
	finalizeKineticEstimator(output_filename);
	finalizeHistogram(output_filename);
...
*/
void finalizePotentialEstimator(string output_filename)
{
	ofstream out(output_filename + "-potential.dat");
	for(int i=0;i<timeslices;i++)
	{
...
void finalizeKineticEstimator(string output_filename)
{
	ofstream out(output_filename + "-kinetic.dat");
	for(int i=0;i<timeslices;i++)
	{
...
void finalizeHistogram(string output_filename)
{
	ofstream out(output_filename + "-probability.dat");
