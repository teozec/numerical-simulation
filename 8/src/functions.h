...
void readInput(std::string input_filename, double temp, double tau);   // reads input from the file "input.dat"
...
projected in a PIGS simulation. We store it in a pointer so we can assign different functions to it in the main.
*/
...
void finalizePotentialEstimator(std::string output_filename);
void finalizeKineticEstimator(std::string output_filename);
void finalizeHistogram(std::string output_filename);
/*
